...
`;
  ...
  const handleSaveSchedule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Conflict validation: Check for overlapping schedules for the EXACT SAME driver AND vehicle
      ...
      const actualConflicts = conflicts?.filter((c) => c.id !== scheduleForm.id) || [];
      if (actualConflicts.length > 0) {
        alert("Conflito de escala! Já existe uma escala agendada para este mesmo motorista e veículo neste intervalo de horário.");
        setSaving(false);
        return;
      }
