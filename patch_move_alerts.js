...
const targetRemoval = alertsStr + `\n\n              {/* Grid split for Issues and Submissions */}`;
const replaceRemoval = `              {/* Grid split for Issues and Submissions */}`;

const targetInjection = `                    {!vehicle.photo_front_url && !vehicle.photo_right_url && !vehicle.photo_left_url && !vehicle.photo_rear_url && (
                      <span className="text-[10px] text-slate-400 italic col-span-4">Nenhuma foto anexada</span>
                    )}
                  </div>
                </div>`;

const replaceInjection = `                    {!vehicle.photo_front_url && !vehicle.photo_right_url && !vehicle.photo_left_url && !vehicle.photo_rear_url && (
                      <span className="text-[10px] text-slate-400 italic col-span-4">Nenhuma foto anexada</span>
                    )}
                  </div>
                </div>
...
if (content.includes(targetRemoval) && content.includes(targetInjection)) {
  content = content.replace(targetRemoval, replaceRemoval);
  content = content.replace(targetInjection, replaceInjection);
