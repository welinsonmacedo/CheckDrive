...
export function parseSpeedKmh(rawSpeed: number | null | undefined): number {
  ...
  // Ignore unrealistic GPS spikes (e.g., 190 km/h or 216 km/h). 
  // Heavy vehicles (trucks) physically do not exceed 130-140 km/h even on extreme downhills.
  // Returning 0 prevents fake speeding alerts from being generated by hardware glitches.
  if (speed > 140) {
    return 0;
  }
...
export function calculateTripMetrics(locations: DriverLocation[]): TripMetrics {
  ...
  for (let i = 0; i < locations.length; i++) {
    ...
    // Sum only valid speeds to calculate an accurate average
  ...
  const avgSpeedKmh = validSpeedCount > 0 ? Math.round(totalSpeedSum / validSpeedCount) : 0;
