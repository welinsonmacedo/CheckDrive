...
export default function ClosingHistoryTab() {
  ...
  const exportToExcel = (closing: any) => {
    ...
    XLSX.writeFile(wb, `Fechamento_${closing.period_start}_a_${closing.period_end}.xlsx`);
  };

  const exportToPDF = (closing: any) => {
    const doc = new jsPDF();
    doc.text(`Relatório de Fechamento: ${formatDate(closing.period_start)} a ${formatDate(closing.period_end)}`, 14, 20);
    doc.setFontSize(10);
  ...
  if (loading) {
     return <div className="p-8 text-center text-xs font-bold text-text-muted">Carregando histórico...</div>;
  }
    ...
    <div className="space-y-4">
      {closings.length === 0 ? (
        <div className="bento-card p-8 flex flex-col items-center justify-center text-center">
          <p className="text-sm font-bold text-text-main mb-2">Nenhum fechamento registrado</p>
          <p className="text-xs text-text-muted">Os saldos resetados nos "Fechamentos de Ciclo" aparecerão aqui.</p>
        </div>
      ) : (
              ...
              >
                <div>
                  <h4 className="text-sm font-black text-text-main">
                    Período: {formatDate(closing.period_start)} a {formatDate(closing.period_end)}
                  </h4>
                  <p className="text-xs text-text-muted mt-1 font-medium">
                    Data do Fechamento: <strong className="font-bold">{dtStr(closing.closed_at)}</strong>
                  </p>
                </div>
                  ...
                  <div className="overflow-x-auto rounded-xl border border-app-border bg-white">
                    <table className="w-full text-left">
                      <thead className="bg-app-bg/50">
                        <tr>
                          <th className="px-4 py-2 text-[10px] font-black text-text-muted uppercase tracking-widest">Motorista</th>
                          <th className="px-4 py-2 text-[10px] font-black text-text-muted uppercase tracking-widest">Checklists Realizados</th>
                          <th className="px-4 py-2 text-[10px] font-black text-text-muted uppercase tracking-widest">Saldo Final</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-app-border">
                        {(closing.score_closing_items || []).map((item: any) => (
                          <tr key={item.id} className="hover:bg-zinc-50">
                            <td className="px-4 py-2.5 text-xs text-text-main font-bold">{item.profiles?.full_name || 'Desconhecido'}</td>
                            <td className="px-4 py-2.5 text-xs text-text-muted font-medium">{item.total_checklists}</td>
                            <td className="px-4 py-2.5 text-xs text-text-main font-black">{item.score} pts</td>
                          </tr>
                        ))}
                        {(!closing.score_closing_items || closing.score_closing_items.length === 0) && (
                           <tr>
                             <td colSpan={3} className="px-4 py-8 text-center text-xs text-text-muted">Nenhum motorista neste fechamento.</td>
                           </tr>
                        )}
                      </tbody>
                    </table>
