...
export default function ChecklistEditModal({ submission, onClose, onSaved }: ChecklistEditModalProps) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedDefects: Record<string, any[]> = { ...submission.details?.defects };
      
      const newIssuesToInsert: any[] = [];
      const issuesToDelete: string[] = []; // issue IDs to delete
      const issuesToUpdate: { id: string, description: string, photo_url: string | null }[] = [];
      ...
      if (submission.type !== 'fuel') {
         for (const [itemId, val] of Object.entries(itemValues)) {
            ...
            if (oldVal === 'defect' && val !== 'defect') {
               const matchingIssues = existingIssues?.filter(issue => issue.item_title.startsWith(titleBase)) || [];
            ...
            if (val === 'defect') {
               const defectsForThisItem = defectData[itemId] || [{ description: '[Editado Manualmente] Defeito adicionado/alterado', photoUrl: null, photoFile: null }];
               ...
               for (let i = 0; i < processedDefects.length; i++) {
                 ...
                 const issueTitle = processedDefects.length > 1 ? `${titleBase} (${i + 1})` : titleBase;
      ...
      const finalStatus = submission.type === 'fuel' ? 'concluido' : (hasDefects ? 'com_defeitos' : 'concluido');
      ...
      if (issuesToDelete.length > 0) {
         await supabase.from('checklist_issues').delete().in('id', issuesToDelete);
      }
                    ...
                    <div key={itemId} className="flex flex-col gap-2 p-3 rounded-xl border border-app-border bg-app-bg/50">
                      ...
                      {value === 'defect' && (
                        <div className="mt-2 animate-in fade-in slide-in-from-top-2 p-3 rounded-xl border border-danger/20 bg-red-50/30 space-y-6">
                           {(defectData[itemId] || [{ description: '', photoUrl: null, photoFile: null }]).map((defect, index) => (
                             <div key={index} className="space-y-4 pb-4 border-b border-danger/10 last:border-0 last:pb-0 relative">
                               {index > 0 && (
                                 <button 
                                   type="button"
                                   onClick={() => {
                                     const newDefects = [...(defectData[itemId] || [])];
                                     newDefects.splice(index, 1);
                                     setDefectData(prev => ({ ...prev, [itemId]: newDefects }));
                                   }}
                               ...
                               <div className="space-y-1.5">
                                 <label className="text-[10px] font-bold text-danger uppercase tracking-widest flex justify-between">
                                   <span>Descrição do Problema #{index + 1}</span>
                                 </label>
                                 <textarea 
                                   ...
                                   onChange={(e) => {
                                     const newDefects = [...(defectData[itemId] || [])];
                                     if (!newDefects[index]) newDefects[index] = { description: '', photoUrl: null, photoFile: null };
                                     newDefects[index] = { ...newDefects[index], description: e.target.value };
                                     setDefectData(prev => ({ ...prev, [itemId]: newDefects }));
                                   }}
                                 />
                               </div>
                               ...
                               <div className="space-y-1.5">
                                 ...
                                 <div className="flex items-center gap-3">
                                   <div className="relative w-16 h-16 rounded-lg border border-red-200 bg-white flex items-center justify-center overflow-hidden">
                                       ...
                                       onChange={(e) => {
                                         ...
                                         if (file) {
                                           imageCompression(file, { maxSizeMB: 0.5 }).then(compressed => {
                                             const newDefects = [...(defectData[itemId] || [])];
                                             if (!newDefects[index]) newDefects[index] = { description: '', photoUrl: null, photoFile: null };
                                             newDefects[index] = { ...newDefects[index], photoFile: compressed };
                                             setDefectData(prev => ({ ...prev, [itemId]: newDefects }));
                                           });
                                         }
                                       }}
                                     ...
                                     {defect.photoFile ? (
                                       <img 
                                         src={URL.createObjectURL(defect.photoFile!)} 
                                   ...
                                   <div className="flex-1">
                                     {defect.photoFile ? (
                                       <p className="text-[10px] font-bold text-red-800 uppercase">Nova Foto Anexada</p>
                                     ) : defect.photoUrl ? (
                                        <p className="text-[10px] font-bold text-red-800 uppercase">Foto Existente</p>
                                     ) : (
                                        <p className="text-[10px] font-bold text-red-800 uppercase">Anexar Foto</p>
                                     )}
                                   </div>
                                 </div>
                               </div>
                           ...
                           <button 
                             type="button"
                             onClick={() => {
                               const newDefects = [...(defectData[itemId] || []), { description: '', photoUrl: null, photoFile: null }];
                               setDefectData(prev => ({ ...prev, [itemId]: newDefects }));
                             }}
