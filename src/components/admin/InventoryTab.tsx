...
export default function InventoryTab() {
  ...
  const handleSaveSupplier = async () => {
    try {
      ...
      if (!supplierForm.id && items.length > 0 && items[0].company_id) {
        payload.company_id = items[0].company_id;
      }

      if (supplierForm.id) {
        const { error } = await supabase.from("inventory_suppliers").update(payload).eq("id", supplierForm.id);
