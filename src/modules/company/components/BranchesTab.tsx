...
`;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setDbError(true);
        // Fallback para LocalStorage para não travar a UI do usuário
        loadLocalData();
      } else if (data) {
        setBranches(data);
      }
  ...
  // Função para buscar dados do CEP automaticamente no ViaCEP
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (!dbError) {
        ...
        // Salva via Fallback LocalStorage
        let updated: Branch[] = [];
        if (form.id) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredBranches.map((branch) => (
            <motion.div
              ...
              className="bg-white rounded-3xl p-5 border border-app-border shadow-sm hover:shadow-md transition-all flex flex-col justify-between space-y-4"
            >
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-sm text-zinc-900 font-sans line-clamp-1">
                        {branch.name}
                      </h3>
