...
export default function ChecklistFlow() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const currentUrlScheduleId = new URLSearchParams(window.location.search).get("schedule");
  ...
  useEffect(() => {
    if (dataRestored) {
      ...
      localforage.setItem(`checklist_state_${type || "start"}`, { formData, currentStep, scheduleId }).catch(err => {
  ...
  const fetchOptions = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        const cacheKey = scheduleId ? `cache_schedule_${scheduleId}` : 'cache_schedules_latest';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      saveOfflineAndExit = async () => {
        setLoading(true);
        const { fileToBase64 } = await import('@/src/lib/offlineSubmitHelper');
        const { queueSubmission } = await import('@/src/lib/offlineQueue');
      ...
      for (const [key, file] of Object.entries(formData.photos)) {
        if (file) {
          const valRes = validateFileUpload(file as File);
          if (!valRes.valid) {
            throw new Error(`Erro na foto externa (${key}): ${valRes.error}`);
          }
      ...
      for (const [itemId, subDefectsRaw] of defectEntries) {
        ...
        for (let i = 0; i < subDefects.length; i++) {
          ...
          if (subDefect.photo) {
            const valRes = validateFileUpload(subDefect.photo);
            if (!valRes.valid) {
              throw new Error(`Erro na foto do defeito (${itemTitle}): ${valRes.error}`);
            }
